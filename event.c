...
int ajouter_event(char *filename, event e)
{
    ...
    // Écriture directe : évite le stack smashing en n'utilisant pas de buffer 'line' local
    // On utilise le séparateur '|' pour la cohérence avec le TreeView
    int result = fprintf(f, "%d|%s|%s|%d|%d|%d|%d|%d|%d|%d|%.2f|%s|%d\n",
            e.id,
            (strlen(e.nom) > 0) ? e.nom : "(nom)",
            (strlen(e.type) > 0) ? e.type : "(type)",
            e.jour, e.mois, e.annee,
            e.heure_debut_h, e.heure_debut_m,
            e.heure_fin_h, e.heure_fin_m,
            (double)e.prix,
            (strlen(e.salle) > 0) ? e.salle : "(salle)",
            e.capacite);
